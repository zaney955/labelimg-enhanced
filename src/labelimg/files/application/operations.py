...
from labelimg.files.infrastructure.qt_trash import (
    is_available as qt_trash_available,
    move_to_trash as move_to_qt_trash,
)
...
def _windows_move_to_recycle_bin(path):
    try:
        from labelimg.platform.trash import delete_to_recycle_bin
...
def _windows_restore_recycle_identity(token, destination):
    try:
        from labelimg.platform.trash import restore_recycle_item
...
class SystemTrashAdapter:
    ...
    def exists(self, identity):
        ...
        if identity.backend == "windows-shell":
            try:
                from labelimg.platform.trash import recycle_item_exists
    ...
    def preflight(self, paths):
        ...
        try:
            if platform.system() == "Windows":
                from labelimg.platform.trash import probe_recycle_support
                for path in paths:
