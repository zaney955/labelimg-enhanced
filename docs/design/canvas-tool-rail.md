...
The Canvas paints one fixed-screen-size marker outside each cluster's union bounds. An amber stacked-box icon identifies likely duplicates, while a magenta font-independent not-equal glyph identifies category conflicts; category conflict dominates when a cluster also contains repeated members of one label. The adjacent number always means total boxes and carries no repeated prefix. A hover tooltip gives the ordered label distribution and visible count, for example `cat ×2, dog ×1 · 2/3 visible`. A fully hidden cluster has no Canvas marker and remains discoverable from the annotation list.
...
The annotation list adds a 10-pixel risk corner to every involved instance button and one unnumbered dominant-risk icon to each affected label group. The Canvas, instance corner, and group indicator share the same not-equal geometry for category conflicts. The group status area reserves a 16-pixel risk slot, a three-pixel gap, and a stable total slot within a 50-pixel column; both icon and `×N` total are vertically centered, and the risk slot remains reserved when empty so totals never shift. Risk activation opens the cluster chooser, while total activation retains ordinary group behavior. Category conflict wins the group icon when both risks occur, while the group tooltip still reports separate cluster counts.

These signals open the same chooser, including for fully hidden clusters. A cluster may be ignored for the current workspace session; member identity, geometry, or label changes invalidate that dismissal, while selection and visibility changes do not. The annotation-list overflow command restores all ignored findings for the current image. No dismissal state is written into Pascal VOC, YOLO, or CreateML documents.
